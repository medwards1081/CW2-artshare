...
const searchIndexName = "artworks"; // ⭐ must match your index name
...
export async function searchArtworks(
  ...
  try {
    ...
    context.error("Artwork search failed:", err);
    return { status: 500, jsonBody: { error: "Search failed" } };
  }
}

app.http("searchArtworks", {
  methods: ["GET"],
  authLevel: "anonymous",
  route: "searchArtworks",
  handler: searchArtworks
});
